...
function update_ratio(WS :: Updating_WorkingSet,
                      wesn_range :: Array{Float64,1}; stratage :: Symbol = :default)
  ...
  if( :default == stratage )
    ...
    WS.ratio[ilon,:] = ratio_lat .* linear_update_ratio(ilat, 
                                     ratio_lat, lat_northforward, lat_southforward ,d_lat)
    WS.ratio[:,ilat] = ratio_lon .* linear_update_ratio(ilon, 
                                     ratio_lon, lon_westforward , lon_eastforward  ,d_lon)
...
function write_ratio_mask(parea :: AnArea_Regular, sst :: Array{Float64,2}; 
                          stratage :: Symbol = :default,
                          wesn_range :: Array{Float64,1} = ones(4)*10.0,
                          whole_map_cir_lon :: Bool = true)
